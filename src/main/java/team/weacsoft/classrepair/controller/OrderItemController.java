...
public class OrderItemController {
    ...
    public Result order(@RequestBody Map<String, Object> payload){
        ...
        try{
            ...
            e.printStackTrace();
            operationLogService.addLog(orderItem.getUserId(), "提交订单失败", EventEnum.ADDORDERITEM.event);
            return ResultFactory.buildFailResult("传递的参数不符合要求");
        }
        operationLogService.addLog(orderItem.getUserId(), "提交订单成功", "-");
    ...
    public Result getOrderItem(HttpServletRequest request){
        ...
        switch (param){
            case "id":
                ...
                break;
            case "userid":
                orderItems = orderItemRepository.findByUserId(Integer.parseInt(value));
                break;
            case "orderid":
            ...
            case "phone":
                ...
                break;
            case "repairid":
                orderItems = orderItemRepository.findByRepairId(value);
                break;
            case "status":
